...
#region//movimiento
	...
	if(estado != eg.atacar){
		if(dire != "") estado = eg.corriendo;
	
		if(vh == 0 and vv == 0) estado = eg.quieto;
	}
	else{
		vh = 0;
		vv = 0;
...
#region//ataques
	if (distance_to_point(objectCA.x, objectCA.y) < radio_ataque and estado != eg.atacar and estado != eg.muriendo)
	{
		estado = eg.atacar;
		image_index = 0;
	}
